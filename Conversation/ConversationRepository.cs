...
namespace Utilities_aspnet.Conversation
{
    ...
    public class ConversationRepository : IConversationRepository
    {
        ...
        {
            var ToUserId = await _context.Set<ConversationEntity>().Where(x => x.FromUserId == userId).Select(x => x.ToUserId).ToListAsync();
            var FromUserId = await _context.Set<ConversationEntity>().Where(x => x.ToUserId == userId).Select(x => x.FromUserId).ToListAsync();
            foreach (var fromUser in FromUserId)
            {
                ToUserId.Add(fromUser);
            }
            var conversations = new List<ConversationsDto>();
            var userIds = ToUserId.Distinct();

            foreach (var item in userIds)
            {
                var user = await _context.Set<UserEntity>().Include(x => x.Media).Select(x => new { x.Id, x.FullName, x.Media }).FirstOrDefaultAsync(x => x.Id == item);
                var conversation = await _context.Set<ConversationEntity>()
