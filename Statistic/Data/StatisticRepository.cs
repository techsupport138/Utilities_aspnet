...
public class StatisticRepository : IStatisticRepository {
    ...
    private readonly List<string> ignoreAction = new(new[] {"read"});
    private readonly List<string> ignoreAgent = new(new[] {"DNTScheduler"});
    private readonly List<string> ignoreController = new(new[] {"partial"});
    private readonly List<string> ignoreReferrer = new(new[] {"localhost:"});
    ...
    public async Task<bool> Add(ActionExecutingContext context) {
        ...
        if (!ignoreController.Contains(controllerName.ToLower()) &&
            ...
            try {
                StatisticEntity? news = new() {
                    //Url = urlName,
                    //Url = context.HttpContext.Request.Path + "/" + context.HttpContext.Request.QueryString,
                    Url =
                        $"{context.HttpContext.Request.Scheme}://{context.HttpContext.Request.Host}{context.HttpContext.Request.Path}{context.HttpContext.Request.QueryString}",
                    Date = DateTime.Now,
                    Hours = (byte) DateTime.Now.Hour,
                    Minutes = (byte) DateTime.Now.Minute,
                    Ip = ip.ToString(),
