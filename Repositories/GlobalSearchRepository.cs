...
public class GlobalSearchRepository : IGlobalSearchRepository {
	...
	public async Task<GenericResponse<GlobalSearchDto>> Filter(GlobalSearchParams filter, string userId) {
		...
		IEnumerable<UserEntity> userList = await _context.Set<UserEntity>()
			.Where(x => x.FullName.Contains(filter.Title) && filter.User).AsNoTracking()
			.Include(u => u.Media)
		...
		IEnumerable<ProductEntity> productList = await _context.Set<ProductEntity>()
			.Where(x => x.Title.Contains(filter.Title) && filter.Product && x.DeletedAt == null).AsNoTracking()
			.Include(i => i.Media)
