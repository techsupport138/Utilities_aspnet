    ...
    public async Task<GenericResponse<GlobalSearchDto>> Filter(GlobalSearchParams filter, string userId)
    {
        ...
        IEnumerable<CategoryEntity> categoryList = await _context.Set<CategoryEntity>().Include(x => x.Users)
            ...
            .Where(x => x.Title.Contains(filter.Title) && filter.Category && x.DeletedAt == null &&
                       (x.Title.Contains(filter.Query) || x.Subtitle.Contains(filter.Query) || x.TitleTr1.Contains(filter.Query) || x.TitleTr2.Contains(filter.Query))
                  )
        ...
        IQueryable<UserEntity> userList = _context.Set<UserEntity>().Where(
            x => x.UserName.Contains(filter.Title) && filter.User ||
                 ...
                 x.AppUserName.Contains(filter.Title) &&
                 filter.User &&
                 (x.FullName.Contains(filter.Query) || x.AppUserName.Contains(filter.Query) || x.FirstName.Contains(filter.Query) || x.LastName.Contains(filter.Query)));
        ...
        IQueryable<ProductEntity> productList = _context.Set<ProductEntity>()
            .Where(x => x.Title.Contains(filter.Title) && filter.Product && x.DeletedAt == null &&
                        (x.Title.Contains(filter.Query) || x.Subtitle.Contains(filter.Query) || x.Description.Contains(filter.Query) || x.Details.Contains(filter.Query)))
