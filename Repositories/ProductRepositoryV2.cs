...
public class ProductRepositoryV2 : IProductRepositoryV2
{
    ...
    public GenericResponse<IEnumerable<ProductEntity>> Filter(ProductFilterDto dto)
    {
        ...
        if (dto.FilterOrder.HasValue)
            products = dto.FilterOrder switch
            {
                ProductFilterOrder.LowPrice => products.OrderBy(x => x.Price).ToList(),
                ProductFilterOrder.HighPrice => products.OrderByDescending(x => x.Price).ToList(),
                ProductFilterOrder.AToZ => products.OrderBy(x => x.Title).ToList(),
                ProductFilterOrder.ZToA => products.OrderByDescending(x => x.Title).ToList(),
                _ => products.OrderBy(x => x.CreatedAt).ToList()
            };
